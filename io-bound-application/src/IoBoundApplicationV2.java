...
public class IoBoundApplicationV2 {
    ...
    private static void performTasks() throws Exception {
        ExecutorService executorService = Executors.newFixedThreadPool(1000);

        try(ExecutorServiceCloseable executorServiceCloseable = new ExecutorServiceCloseable(executorService)) {
            for (int i = 0; i < NUMBER_OF_TASKS; i++) {
                executorService.submit(new Runnable() {
                    @Override
                    public void run() {
                        // 100 tasks involve 99 context switches
                        // overhead of the context switches
